...
using keyleds::device::LogitechDevice;
using keyleds::device::LogitechDeviceWatcher;
...
LogitechDevice::LogitechDevice(std::unique_ptr<struct keyleds_device> device,
...
std::unique_ptr<LogitechDevice> LogitechDevice::open(std::string path)
{
    ...
    return std::unique_ptr<LogitechDevice>(new LogitechDevice(
...
LogitechDevice::Type LogitechDevice::getType(struct keyleds_device * device)
{
...
std::string LogitechDevice::getName(struct keyleds_device * device)
{
    char * name;
...
void LogitechDevice::parseVersion(struct keyleds_device * device, std::string * model,
...
LogitechDevice::block_list LogitechDevice::getBlocks(struct keyleds_device * device)
{
...
bool LogitechDevice::hasLayout() const
{
    return layout() != KEYLEDS_KEYBOARD_LAYOUT_INVALID;
}

std::string LogitechDevice::resolveKey(key_block_id_type blockId, key_id_type keyId) const
{
...
int LogitechDevice::decodeKeyId(key_block_id_type blockId, key_id_type keyId) const
{
...
void LogitechDevice::setTimeout(unsigned us)
{
    keyleds_set_timeout(m_device.get(), us);
}

void LogitechDevice::flush()
{
...
bool LogitechDevice::resync() noexcept
{
...
void LogitechDevice::fillColor(const KeyBlock & block, const RGBColor color)
{
...
void LogitechDevice::setColors(const KeyBlock & block, const ColorDirective colors[], size_t size)
{
...
void LogitechDevice::getColors(const KeyBlock & block, ColorDirective colors[])
{
...
void LogitechDevice::commitColors()
{
...
LogitechDevice::error::error(std::string what, keyleds_error_t code, int oserror)
 : Device::error(what), m_code(code), m_oserror(oserror)
{
    if (m_code == KEYLEDS_ERROR_ERRNO && m_oserror == 0) { m_oserror = errno; }
}

LogitechDevice::error::~error() {}

bool LogitechDevice::error::expected() const
{
...
bool LogitechDevice::error::recoverable() const
{
...
LogitechDeviceWatcher::LogitechDeviceWatcher(struct udev * udev, QObject *parent)
 : FilteredDeviceWatcher(udev, parent)
{
    setSubsystem("hidraw");
}

LogitechDeviceWatcher::~LogitechDeviceWatcher() {}

bool LogitechDeviceWatcher::isVisible(const ::device::Description & dev) const
{
